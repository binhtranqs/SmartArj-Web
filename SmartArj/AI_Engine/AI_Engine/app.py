...
def _get_zone_id(cursor, city_id: int) -> int:
    cursor.execute(
        "SELECT TOP 1 ZoneID FROM Zones WHERE CityID = ? ORDER BY ZoneID", city_id
    )
    row = cursor.fetchone()
...
def save_forecasts_to_db(city_name: str, target: str, forecast: list) -> dict:
    ...
    try:
        ...
        for item in forecast:
            ...
            cursor.execute(
                "SELECT ForecastID FROM Forecasts WHERE CityID = ? AND ForecastDate = ?",
                city_id, date
            )
            ...
            if existing:
                if target == "Temperature":
                    cursor.execute(
                        "UPDATE Forecasts SET Temperature = ?, CreatedAt = GETDATE() WHERE ForecastID = ?",
                        value, existing[0]
                    )
                else:
                    cursor.execute(
                        "UPDATE Forecasts SET Humidity = ?, CreatedAt = GETDATE() WHERE ForecastID = ?",
                        value, existing[0]
                    )
                updated += 1
            else:
